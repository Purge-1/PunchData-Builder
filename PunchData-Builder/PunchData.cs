...
namespace PunchData_Builder
{
    public class PunchData
    {
        ...
        public void Build()
        {
            ...
            var Path2 = "output/punch_data.txt";
            using var fs = File.OpenWrite(Path);
            using var fs2 = File.OpenWrite(Path2);
            using var bs = new BinaryWriter(fs);
            using var bs2 = new BinaryWriter(fs2);
            bs.Write(Version);
            ...
            bs2.Write(Version);
            bs2.Write(Items.Count);
            foreach(var item in Items)
            {
                bs2.Write(item.ItemId);
                bs2.Write(item.EffectId);
            }
            bs2.Close(); fs2.Close();

            Console.WriteLine($"[~] Successfully on writting punch_data.dat");
            Console.WriteLine($"[~] Successfully written punch_data.txt");
            Console.WriteLine($"[~] Made by Peachy/Purge.");
            Console.WriteLine($"[~] Built in {DateTime.Now.ToString("d/MM/yyyy/h/m")}");
            Console.WriteLine($"[~] {Items.Count} Is added into the list!");
            Console.WriteLine($"[~] Version {Version}");
            Console.WriteLine($"[~] {String2} Mins Project");
        }
    }
}
